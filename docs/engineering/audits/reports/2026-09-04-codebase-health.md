...
| Files > 600 lines | 89 | — | — |
| Functions > 80 lines (heuristic) | 185 (66 > 200, 20 > 400) | — | — |
| Dead exports (zero refs outside file, runtime kinds) | 299 (+ 518 unused interface/type exports) | — | — |
| Files with zero importers | 10 real + 2 intentional | — | — |
| Unused locals/params (`tsc` audit config) | 133 | — | — |
| Unused CSS classes (no TS producer, heuristic) | ~473 | — | — |
| Unused `en.ts` i18n keys | ~160 of 1,921 | — | — |
| ESLint (`eslint src`, full config) | 328 problems, 313 of them `no-unsafe-*` | — | — |
...
| Tests that assert on source text (`readFileSync` of `.ts/.css`) | 31 files, 245 reads | — | — |
...
### CH-2026-09-04-#1 — Community project sync transmits private book titles; projection layer is bypassed

- **Status:** Confirmed
- **Category:** doctrine correction
- **Severity:** ORANGE
- **Confidence:** High
- **Risk:** Author trust. `docs/engineering/standards/writing-session-privacy.md:65` says `bookTitle` is opt-in per book and `:141` says never substitute "Untitled". The wire path does the opposite and the doc's `:9` claim that only the `private` audience ships is stale (daily sync has shipped since 2026-07-16).
- **Effort:** 2–4 h for the title fix + test flip; 1 day to re-point the tracer test at the real path
- **Evidence:**
  - `src/communityShare/communityShareClient.ts:554` — `book.publicLabel || book.title || 'Untitled book'` is sent on every project sync. Gate at `:590` checks only `enabled/connected/!paused`, not tier. Fires from `src/main.ts:720` at startup and from `src/settings/sections/ProgressSection.ts:847,889,940` on every target-date edit. `stageTargetDates` and `enableZeroDraftMode` also ship and appear in no field policy or manifest.
  - `src/communityShare/communityShareClient.test.ts:579` codifies the leak: `expect(body.projects[1].title).toBe('Second Book')` for a book with no `publicLabel`.
  - The preview path deliberately uses `publicLabel` only (`src/communityShare/communitySharePreview.ts:110-112`), so the two paths disagree.
  - The doc comment at `communityShareClient.ts:523-527` and `:538-540` says shells "arrive PRIVATE" server-side. That is a server-side privacy model; the doctrine describes a device-side one. One of them has to change.
...
- **Suggested next action:** Owner decision on the privacy model. If device-side (per doctrine): send `publicLabel` only, omit when absent, gate project sync on tier ≥ 2, flip the test at `:579`. If server-side: amend the doctrine to say so explicitly and add `book.title` / target dates to the field manifest. Either way, point the tracer test (all five TRACER strings) at `buildCommunityDailyEntries` + `buildCommunitySharePreview`, and delete or adopt the unused projection functions.
...
- **Severity:** ORANGE
- **Confidence:** High
- **Risk:** Untestable by construction (the only tests on these files assert source text); inverted dependency `services/* → settings/sections/*`; the same logic cannot be reached from commands without a settings tab.
...
- **Evidence:**
  ...
  - 299 exported functions/consts/classes with zero references outside their file (scan: `scratchpad/dead-exports.txt`; sample of 25 verified by `grep -rlw` at 0 hits each). By dir: inquiry 63, utils 59, renderer 55, ai 27, settings 13. Named examples: `constants/inquiryUi.ts:56-63` all eight `getInquiryHelp*Tooltip`; `utils/manuscript.ts:530` `estimateTokens` (words-based, a second token heuristic); `ai/runtime/runtimeSelection.ts:23` `resolveAccessTier` (the *canonical* one — unused while 5 private copies live); `providers/provider.ts:11` `defaultCapabilitiesForProvider`; `constants/aiDefaults.ts:5-6` `DEFAULT_OPENAI_MODEL_ID`/`DEFAULT_ANTHROPIC_MODEL_ID`; `utils/exportFormats.ts:53,1515,1543,1600`; `utils/logVaultOps.ts:136 trashFiles`; `SettingsTab.showPathSuggestions:348-388`, `renderProHero:751-769`; `PublishSection.ts:1676` `addProRow = (s) => s` identity wrapper applied 13×.
...
- **Suggested next action:** One commit: `git rm` the two ` 2.ts` files and the 10 dead files, delete the named zero-reference exports and 8 settings fields, run `tsc` + `vitest`. Then re-run the dead-export scan (the barrel currently hides drift). Add `npm run audit:tsc-unused` to `gates` as a ratchet.
...
- **Evidence (family → copies):**
  ...
  - `resolveAccessTier`: canonical `ai/runtime/runtimeSelection.ts:23` (0 callers); copies `aiClient.ts:60`, `registry/recommendations.ts:32`, `inquiry/services/inquiryModelResolver.ts:105`, `InquiryView.ts:1505`, `AiSection.ts:916`.
  ...
  - `svgToPngBuffer`: byte-identical `services/export/TimelineExportService.ts:771-812` and `services/apr/AuthorProgressRenderService.ts:496-535`.
...
- **Suggested next action:** Start with the ones that have a behavioural bug: add `formatLocalDateKey`/`parseLocalDateKey` (null on bad input) to `utils/date.ts`; pick one `normalizeAngle` contract; move `svgToPngBuffer` to `services/export/`; make `runtimeSelection.resolveAccessTier` the only one. Each is an independent commit with `tsc` + `vitest`.
...
- **Evidence:** `PublishSection.renderPublishSection` 2,843 (`:1660-4502`); `AuthorProgressSection.renderAuthorProgressSection` 1,892 (`:59-1950`); `InquirySection.renderInquirySection` 1,675 (`:160-1834`); `ChronologueShiftController.setupChronologueShiftController` 1,028 (`:85-1112`, 23 nested closures); `BeatPropertiesSection.renderAuditPanel` 1,548 (`:3721-5268`); `GossamerCommands.runGossamerAiAnalysis` 707 (`:751-1457`); `SynopsisManager.generateElement` 673; `CommandRegistrar.handleManuscriptExport` 642 (`:329-970`, 27 Notices, 0 early returns); `CommunityShareSection.renderCommunityShareSection` 635; `InquiryView.buildZoneGradients` 486 (`:2368-2853`, 333 numeric literals, no `this` state beyond `rootSvg`); `InquiryView.runInquiry` 333; `aiClient.run` 483; `TimeLineView.ensureBookSwitcher` 412; `main.loadSettings` 386 (a migration pipeline with no section structure).
...
- **Effort:** 1 day
- **Evidence:** `docs/engineering/plans/v7-removals.md` is `status: pending`, target 7.0.0; package is 7.2.0. `TODO(v7)` ×3 at `src/authorProgress/authorProgressConfig.ts:67, 84, 309`. `src/migrations/beatSettings.ts` (436 lines) scheduled for deletion, still called from `main.ts:1139, 1361`. `src/migrations/sceneAnalysis.ts` (v5-era) runs at `main.ts:582` on every load. 44 `@deprecated` fields remain across `types/settings.ts`/`defaults.ts`. Half-finished deprecations: `targetCompletionDate` (`settings.ts:1076`) still read as a runtime fallback in `renderer/components/ProgressTicks.ts:150`, `renderer/ChangeDetection.ts:259`, `utils/sceneHelpers.ts:193`, `services/export/TimelineExportService.ts:104,257`; `synopsisHoverMaxLines` (`:1113` deprecated) still **written** by `AiSection.ts:4163` and read by `SceneAnalysisProcessingModal.ts:621`; `outlineOutputFolder` re-synced by `ConfigurationSection.ts:92`; `aiOutputFolder` ("no longer user-configurable") still has a settings row `ConfigurationSection.ts:65-66` + 5 locale strings. `main.ts:1265, 1274` reads `stageTargetDates` flagged `@deprecated` by eslint. `InquiryArtifactStore.ts:104-138` `migrateInquirySidecarToVisible` (copy-then-delete of user data) has **zero tests**. `InquiryRunnerService.ts:115-121, 2072-2077` accepts a legacy nested `verdict` wire shape alongside the flat one.
...
- **Severity:** YELLOW
- **Confidence:** High
- **Risk:** Three inconsistent XML escapers mean an unescaped scene title is an XML injection into the timeline; the "pure" renderer is non-deterministic for identical inputs.
...
- **Evidence:** 31 test files do `readFileSync` of `src/**/*.ts` or `.css` (245 reads; `settings/sections` 64, `InquiryView.ts` 54, `inquiry/runner` 13). `AiSection.test.ts` has 41 `it` / 53 source reads / 0 behavioural cases; `InquiryView.test.ts` is 86 source-string assertions. Zero behavioural coverage of `runInquiry`, omnibus, pending-edits writeback, `buildCorpusEntryList`. No test file at all: `BeatPropertiesSection.ts` (5,788), `AuthorProgressSection.ts`, `ProgressSection.ts`, `CommunityShareSection.ts`, `OnboardingModal.ts` (1,453), `OnboardingService.ts` (934), `WelcomeScreen.ts`, `ChronologueShiftController.ts` (1,882), `TimelineRenderer.ts`, `gossamerLayer.ts`, `SceneInteractionManager.ts`, `RendererService.ts`, `main.ts`, `SynopsisManager.ts`; `src/modes` 8 files / 0 tests; `src/ui` 5 / 0; `src/sceneAnalysis` 12 / 2 (the `FileUpdater` YAML-write path has none). Pure and untested (cheapest wins): `utils/sceneHelpers.ts` (427, 21 exports incl. all scene sorters), `utils/text.ts` (289, scene-title/number canonical), `utils/beatsSystems.ts` (571), `utils/planetaryTime.ts`, `utils/runtimeEstimator.ts`. Untested community-client exits: `syncCommunityShareIfDue`, `beginCommunitySharing`, `pauseCommunitySharing`, `resumeCommunitySharing`, `postSessionToCommunityFeed`. Test helpers copied: `function makeFile` ×14 files, `createPlugin` ×8, `makePlugin` ×5. `vitest.config.ts` has no `restoreMocks`/`clearMocks`. 20 test files use real `Date.now()`/`new Date()` without fake timers (mostly relative fixtures; flakiness Hypothesis). Certification: `anthropicCertification.test.ts:213` is `skipIf`; last report `docs/audits/anthropic-certification.json` is 2026-04-15 against `claude-sonnet-4-6`, no longer in the registry.
- **Suggested next action:** Add vitest for `sceneHelpers.ts`, `text.ts`, `beatsSystems.ts` before #6 touches them; hoist `makeFile`/`createPlugin` into `tests/helpers/`; re-run `npm run certify:anthropic`.
...
- **Author trust:** Concern — CH-#1 (private titles transmitted; doctrine says opt-in) and CH-#7 (cost numbers that can be silently wrong).
...
- → **Owner decision (not an audit):** CH-#1 privacy model (device-side vs server-side), CH-#8 hover filters.
...
- **Run on:** 2026-09-11
- **Specific things to re-check:** whether `audit:tsc-unused` was added to `gates`; dead-export count after the `ai/index.ts` barrel is removed (it currently hides drift); CSS producer-less count after the `modal.css` block deletion; `.gate-logs` size; whether the `v7-removals.md` status changed; `TimeLineView.renderTimeline` registration count per render.
...
Scan artefacts (dead exports, unused CSS, unused i18n keys, long-function list) were generated in the session scratchpad and are reproducible from the commands in the findings; they were not committed.
