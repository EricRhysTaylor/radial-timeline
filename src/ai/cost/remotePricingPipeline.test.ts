...
describe('remote pricing pipeline — happy path', () => {
    ...
    it('fetched remote pricing is merged and drives active pricing table', async () => {
        const remotePayload = {
            models: [
                { provider: 'anthropic', modelId: 'claude-opus-5', inputPer1M: 2.5, outputPer1M: 12.0 },
                { provider: 'openai', modelId: 'gpt-6-sol', inputPer1M: 2.8, outputPer1M: 9.0 }
            ]
        };
        ...
        expect(getProviderPricing('anthropic', 'claude-opus-5').inputPer1M).toBe(2.5);
        expect(getProviderPricing('openai', 'gpt-6-sol').inputPer1M).toBe(2.8);
    ...
    it('new remote-only model appears in active pricing after merge', async () => {
        ...
        expect(getProviderPricing('anthropic', 'claude-opus-5').inputPer1M).toBe(5.0);
    });

    it('remote pricing overrides builtin pricing for same model', async () => {
        const original = getProviderPricing('openai', 'gpt-6-sol');
        expect(original.inputPer1M).toBe(2.0);

        mergeRemotePricing({
            openai: { 'gpt-6-sol': { inputPer1M: 1.5, outputPer1M: 7.0 } }
        }, 'remote', new Date().toISOString());

        expect(getProviderPricing('openai', 'gpt-6-sol').inputPer1M).toBe(1.5);
        expect(getProviderPricing('openai', 'gpt-6-sol').cacheReadPer1M).toBe(0.2);
    });
    ...
    it('cache is written on successful remote fetch', async () => {
        ...
        await loadRemotePricing({
            ...
            fetchImpl: mockFetch({
                models: [{ provider: 'openai', modelId: 'gpt-6-sol', inputPer1M: 3, outputPer1M: 10 }]
            })
        });
        ...
        expect(parsed.table.openai?.['gpt-6-sol']).toBeDefined();
    });
});
...
describe('remote pricing pipeline — fallback behavior', () => {
    ...
    it('uses stale cache when remote fetch returns non-ok', async () => {
        const result = await loadRemotePricing({
            ...
            readCache: async () => staleCacheJson({
                table: { anthropic: { 'claude-opus-5': { inputPer1M: 99, outputPer1M: 99 } } }
            }),
        ...
        expect(result.table?.anthropic?.['claude-opus-5']?.inputPer1M).toBe(99);
    ...
    it('falls back to cache when fetch throws a network error', async () => {
        const result = await loadRemotePricing({
            ...
            url: 'https://example.com/pricing.json',
            readCache: async () => staleCacheJson({
                table: { openai: { 'gpt-6-sol': { inputPer1M: 42, outputPer1M: 42 } } }
            }),
            writeCache: async () => undefined,
    ...
    it('partial remote failure does not break pricing: builtin models remain after null merge', async () => {
        ...
        const opus = getProviderPricing('anthropic', 'claude-opus-5');
    ...
    it('uses fresh cache without fetching (default 24h TTL)', async () => {
        ...
        const result = await loadRemotePricing({
            ...
            readCache: async () => freshCacheJson({
                table: { openai: { 'gpt-6-sol': { inputPer1M: 77, outputPer1M: 77 } } }
            }),
            writeCache: async () => undefined,
        ...
        expect(result.source).toBe('cache');
        expect(result.table?.openai?.['gpt-6-sol']?.inputPer1M).toBe(77);
...
describe('cache corruption edge cases', () => {
    ...
    it('writeCache failure during pricing fetch returns remote data safely', async () => {
        ...
        expect(result.source).toBe('remote');
        expect(result.table?.openai?.['gpt-6-sol']?.inputPer1M).toBe(3);
