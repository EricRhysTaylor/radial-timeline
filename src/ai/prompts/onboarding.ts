...
export function getOnboardingSurveyInstructions(): string {
  return [
    ...
    '  thread hangs from.',
    '- NEVER create a subplot that restates the main plot under another name (e.g. "The',
    '  Return of Ulysses" when the return IS the main plot) — those scenes belong to',
    '  "Main Plot" itself, which should end up carrying a substantial share of scenes.',
...
export function getOnboardingSceneInstructions(): string {
  return [
    ...
    'When a scene advances the CENTRAL storyline, assign "Main Plot" — reserve the named threads for',
    'scenes whose primary work is that specific thread. Main Plot is the spine, not a leftover bucket.',
