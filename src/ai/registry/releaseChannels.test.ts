...
describe('release channel curation', () => {
    ...
    it('returns both curated Google picker entries (depth + speed)', () => {
        ...
        expect(picker).toContain('gemini-3.8-flash');
    });
    ...
    it('returns the current Anthropic model, the premium pro entry, and the one-back continuity model', () => {
        ...
        //   - Opus 5.5: newest stable, the auto-selected default, offered first.
        //   - Fable 5.1: the 'pro'-channel premium model — visible and pinnable
        //     but never the silent default (it is 2.5× Opus cost).
        ...
        // product decision.
        expect(picker.slice(0, 2)).toEqual(['claude-opus-5-5', 'claude-fable-5-1']);
        //   - Opus 5: continuity opt-in so in-flight authors aren't force-migrated.
    ...
    it('keeps Claude Fable 5.1 off the stable channel so latest-stable stays Opus 5.5', () => {
        const stable = selectLatestModelByReleaseChannel(BUILTIN_MODELS, 'anthropic', 'stable');
        expect(stable?.alias).toBe('claude-opus-5-5');
