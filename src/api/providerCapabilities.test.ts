...
describe('sanitizeDispatchParams', () => {
    it('strips OpenAI GPT-6 Sol sampling controls before dispatch', () => {
        ...
        expect(result.notes).toContain('Stripped temperature for openai/gpt-6-sol: model does not support temperature');
        expect(result.notes).toContain('Stripped topP for openai/gpt-6-sol: model does not support topP');
    });

    // Note: with the minimum-viable catalog (2026-05-22), GPT-6 Sol is the
    ...
    it('centralizes GPT-6 Sol request profile metadata for provider adapters', () => {
        expect(getModelRequestProfile('openai', 'gpt-6-sol')).toMatchObject({
