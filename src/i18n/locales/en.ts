    ...
    settings: {
        ...
        ai: {
            ...
            localLlm: {
                ...
                validationDeadline: 'Local LLM validation timed out after 60s. The server accepted the connection but never finished a request. Check: (1) a large model may still be loading — first request after starting a server can take 30-60s, so try again; (2) if the Base URL says "localhost", try "127.0.0.1" instead — macOS may resolve localhost to IPv6 while the server listens on IPv4 only; (3) confirm the server is still running.',
            ...
            localLlmConfig: {
                ...
                jsonModeDesc: 'How structured JSON output is requested from the local server. "Response format" makes the server enforce the shape, which is the safest option but can be several times slower — constraining generation is expensive on some servers. "Prompt only" asks for the shape in the prompt instead; Radial Timeline still validates every reply and rejects malformed ones. Run diagnostics below to see which is faster on your machine, and whether your model produces valid JSON without enforcement.',
                optionJsonModeResponseFormat: 'Response format (server-enforced)',
                optionJsonModePromptOnly: 'Prompt only (usually faster)',
                ...
                capabilityManuscriptScaleName: 'Manuscript-scale work',
                capabilityManuscriptScaleDesc: 'The model keeps a manuscript-sized prompt coherent across its full context window, and emits long structured responses without truncating mid-object. Required by Gossamer and Inquiry, which need both together.',
            },
            config: {
        ...
        inquiry: {
            ...
            prompts: {
                ...
                unlockProGhost: 'More custom question slots are part of Pro',
            ...
            canonicalLibrary: {
                ...
                descPro: 'Frame Inquiry across three zones: Setup, Pressure, and Payoff. Add your own custom questions, install curated questions line by line, or load the full extended set across all zones at once.',
                ...
                loadFullProSet: 'Load Full Extended Set',
        ...
        authorProgress: {
            ...
            publishing: {
                ...
                proTeaser: {
                    wantMore: 'Looking for Campaign manager?',
                    enhanceWorkflow: 'A Pro workflow',
                    desc: 'Campaign manager lets you create multiple embeds with Teaser Reveal\u2014progressively show more detail as you write. It sits with the other Pro workflows, alongside runtime (RT) chronologue mode and advanced Pandoc publishing templates.',
                    upgradeLink: 'Open Settings \u2192 Pro',
                },
            },
            progressMode: {
