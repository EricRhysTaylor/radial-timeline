...
import { SceneTimeService } from './sceneTime/SceneTimeService';
import { sceneTimeEditorExtension } from './sceneTime/editorRuler';
import { renderReadingTime } from './sceneTime/readingRuler';
...
export default class RadialTimelinePlugin extends Plugin {
    ...
    public async setTimelineScope(scope: 'book' | 'saga'): Promise<void> {
        if (scope === 'saga') {
            ...
            if (!hasProFeatureAccess(this)) {
                new Notice('Saga Timeline is a Pro workflow.');
                return;
            }
    ...
    async onload() {
        ...
        // Resolve the vault-local Pandoc font destination. Bundled templates
        // point at Radial Timeline/Pandoc/fonts, not MacTeX, Font Book, or
        // Google URLs. The font bytes themselves are embedded in main.js
        // (src/utils/embeddedAssets.ts) — there is no plugin-folder source
        // path to resolve, because Obsidian never installs one.
        try {
