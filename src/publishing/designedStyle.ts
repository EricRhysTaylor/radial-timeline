...
/**
 * v3 added `parts.title` — whether a layout prints an explicit Part marker's
 * title alongside its numeral. Stored v2 specs are upgraded by
 * `migrateDesignedStyleSpec`, which defaults the flag to false so a layout
 * authored before titles existed keeps printing numerals only.
 */
export const DESIGNED_STYLE_SPEC_VERSION = 3 as const;
...
export interface DesignedStyleSpec {
    ...
    parts:    {
        ...
        pageBreak: boolean;
        /**
         * Print an explicit Part marker's title under the numeral.
         *
         * Independent of the macro's arity: `\rtPart` always takes the title
         * argument, because the export always emits it. This only decides
         * whether the layout typesets it. Absent/false is numerals-only, which
         * is what every layout authored before Parts became explicit markers
         * expects.
         */
        title?: boolean;
        epigraph: boolean;
...
/** A stored spec before normalization: the current shape, minus version certainty. */
type StoredDesignedStyleSpec = Omit<DesignedStyleSpec, 'specVersion'> & { specVersion: number };

/**
 * Bring a persisted spec up to the current version.
 *
 * Stored specs are read back as `JSON.parse(...) as DesignedStyleSpec`, so
 * TypeScript believes every one of them carries the current `specVersion` while
 * the JSON on disk still says whatever version wrote it. Without normalization
 * that lie surfaces: the generator stamps `spec.specVersion` into the .tex
 * header comment, so a v2 spec regenerated by a v3 generator would claim to be
 * v2 output.
 *
 * Upgrades are additive and lossless — a v2 spec simply had no opinion about
 * part titles, and numerals-only is what it rendered.
 *
 * Returns the same object when it is already current, so callers can use the
 * identity check to decide whether anything needs persisting.
 */
export function migrateDesignedStyleSpec(spec: DesignedStyleSpec): DesignedStyleSpec {
    const stored = spec as StoredDesignedStyleSpec;
    if (stored.specVersion === DESIGNED_STYLE_SPEC_VERSION && typeof spec.parts.title === 'boolean') {
        return spec;
    }
    ...
    return {
        ...
        parts: {
            ...spec.parts,
            // v2 and earlier predate titled parts; numerals-only preserves their output.
            title: spec.parts.title ?? false,
        },
    };
}
