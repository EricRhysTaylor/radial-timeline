...
describe('AI settings models table', () => {
    ...
    it('surfaces provider usage cost in the AI model preview when exact usage pricing is available', () => {
        const source = readFileSync(resolve(process.cwd(), 'src/settings/sections/AiSection.ts'), 'utf8');
        expect(source.includes('estimateUsageCost(context.provider, context.modelId, latestSession.result.tokenUsage, latestSession.providerCacheStatus, ANTHROPIC_REQUESTED_CACHE_TTL)')).toBe(true);
